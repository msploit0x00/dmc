...
//                 // ✅ IMPROVED Sales Order reference finding
...
//                     // ✅ DEBUG: Log all existing rows first
//                     console.log('🔍 All existing rows:', frm.doc.items.map(item => ({
...
//                     console.log('🔍 Looking for existing row with:', {
//                         itemCode: itemCode,
//                         batchNo: batchNo,
//                         barcode: barcode
//                     });

//                     // ✅ Check for existing NON-FREE row with same item + batch (REMOVE barcode requirement)
//                     const existingRow = frm.doc.items.find(item =>
//                         item.item_code === itemCode &&
//                         item.batch_no === batchNo &&
//                         !item.is_free_item // ✅ Only match non-free items
//                         // ✅ REMOVED barcode check - same item+batch should always increment
//                     );

//                     console.log('🔍 Found existing row:', existingRow ? {
//                         item_code: existingRow.item_code,
//                         qty: existingRow.qty,
//                         barcode: existingRow.barcode,
//                         batch_no: existingRow.batch_no
//                     } : 'NONE');

//                     const handleRateAndUpdate = (correctRate, isFreeItem = false) => {
//                         console.log('handleRateAndUpdate called with rate:', correctRate, 'isFreeItem:', isFreeItem);

//                         // ✅ Force rate to 0 for free items
//                         const finalRate = isFreeItem ? 0 : (correctRate > 0 ? correctRate : 0);

//                         if (existingRow) {
//                             // ✅ SUPER AGGRESSIVE concurrency control
//                             frm._updating_from_scan = true;
//                             frm._force_no_triggers = true;

//                             // ✅ SAME ITEM+BATCH - increment quantity
//                             const newQty = (existingRow.qty || 0) + 1;
//                             console.log('🔄 SAME ITEM+BATCH detected, incrementing:', existingRow.qty, '→', newQty);
//                             let newStockQty;
...
//                             if (isUnitUOM) {
//                                 newStockQty = newQty;
//                                 console.log('📦 Unit UOM - stock_qty = qty:', newStockQty);
//                             } else {
//                                 const convFactor = existingRow.conversion_factor || conversionRate || 1;
//                                 newStockQty = newQty * convFactor;
//                                 console.log('📦 Non-Unit UOM - stock_qty = qty * conversion:', newQty, '*', convFactor, '=', newStockQty);
//                             }

//                             // ✅ 🔥 FIXED AMOUNT CALCULATION - Based on UOM type
//                             let newAmount;
//                             if (isFreeItem) {
//                                 newAmount = 0;
//                             } else {
...
//                                     newAmount = finalRate * newQty;
//                                     console.log('💰 Unit UOM - amount = rate * qty:', finalRate, '*', newQty, '=', newAmount);
//                                 } else {
//                                     // Non-Unit UOM: amount = rate * stock_qty
//                                     newAmount = finalRate * newStockQty;
//                                     console.log('💰 Non-Unit UOM - amount = rate * stock_qty:', finalRate, '*', newStockQty, '=', newAmount);
//                                 }
//                             }

//                             console.log('🔄 Updating existing row:', {
//                                 item: itemName,
//                                 oldQty: existingRow.qty,
//                                 newQty: newQty,
...
//                             // ✅ DIRECT field assignment to prevent conflicts
...
//                             // 🔥 STORE CUSTOM AMOUNT for aggressive protection
//                             if (!frm._custom_amounts) frm._custom_amounts = {};
//                             frm._custom_amounts[existingRow.name] = newAmount;

//                             // ✅ FORCE update using frappe.model.set_value with delay
//                             setTimeout(() => {
//                                 Promise.all([
...
//                                 ]).then(() => {
//                                     // 🔥 AUTOMATIC SCAN CALCULATION - PROPER SEQUENCE
//                                     console.log('🔄 Starting automatic scan calculations...');

//                                     // STEP 1: Ensure amount is correct first
//                                     setTimeout(() => {
//                                         existingRow.amount = newAmount;
//                                         frappe.model.set_value(existingRow.doctype, existingRow.name, 'amount', newAmount);

//                                         // STEP 2: Update totals after amount is fixed
//                                         setTimeout(() => {
//                                             update_total_qty(frm);
//                                             calculate_document_totals(frm);
...
//                                                 // STEP 4: Fix taxes with UOM logic after totals are correct
//                                                 setTimeout(() => {
//                                                     refresh_taxes_on_net_total_change(frm, frm.doc.net_total);

//                                                     console.log('✅ Scan calculations complete - Amount, Totals, and Taxes fixed');
...
//                                                     frappe.show_alert({
//                                                         message: __(`Updated quantity to ${newQty} (Stock: ${newStockQty}, Amount: ${newAmount}) for ${itemName}`),
//                                                         indicator: 'blue'
//                                                     });
//                                                 }, 300);
//                                             }, 200);
...
//                         // ✅ Create new row with improved logic
//                         frm._updating_from_scan = true;
//                         frm._force_no_triggers = true;

//                         const newRow = frm.add_child('items');
//                         // ✅ Calculate stock_qty based on UOM type
//                         const uomLower = (uom || '').toLowerCase().trim();
//                         const isUnitUOM = ['unit', 'units', 'nos', 'pcs', 'piece', 'pieces', 'each'].includes(uomLower);
//                         const stockQty = isUnitUOM ? 1 : (conversionRate || 1);

//                         console.log('📦 New row stock_qty calculation:', {
//                             uom: uom,
//                             isUnitUOM: isUnitUOM,
...
//                                 // Unit UOM: amount = rate * qty
//                                 amount = finalRate * 1;
//                                 console.log('💰 New Row Unit UOM - amount = rate * qty:', finalRate, '* 1 =', amount);
//                             } else {
//                                 // Non-Unit UOM: amount = rate * stock_qty
//                                 amount = finalRate * stockQty;
//                                 console.log('💰 New Row Non-Unit UOM - amount = rate * stock_qty:', finalRate, '*', stockQty, '=', amount);
//                             }
//                         }

//                         console.log('🆕 FIRST SCAN - Creating new row:', {
//                             item: itemName,
//                             qty: 1,
//                             stockQty: stockQty,
//                             rate: finalRate,
...
//                             frappe.model.set_value(newRow.doctype, newRow.name, 'custom_expiry_date', expiryDate),
//                             frappe.model.set_value(newRow.doctype, newRow.name, 'barcode', barcode),
//                             frappe.model.set_value(newRow.doctype, newRow.name, 'rate', finalRate),
//                             frappe.model.set_value(newRow.doctype, newRow.name, 'amount', amount),
//                             frappe.model.set_value(newRow.doctype, newRow.name, 'custom_out_qty', 1),
//                             frappe.model.set_value(newRow.doctype, newRow.name, 'is_free_item', isFreeItem ? 1 : 0)
//                         ]).then(() => {
//                             // ✅ Set Sales Order reference if available
//                             if (salesOrderRef) {
//                                 Promise.all([
//                                     frappe.model.set_value(newRow.doctype, newRow.name, 'against_sales_order', salesOrderRef),
//                                     frappe.model.set_value(newRow.doctype, newRow.name, 'so_detail', so_detail_id || "")
//                                 ]).then(() => {
//                                     // ✅ Set Sales Order reference only if field exists
//                                     if (salesOrderRef && frm.fields_dict.sales_order) {
//                                         if (!frm.doc.sales_order) {
//                                             frm.set_value('sales_order', salesOrderRef);
//                                         }
//                                     }
//                                 });
//                             }

//                             // 🔥 SUPER AGGRESSIVE AMOUNT PROTECTION for new rows
//                             protect_amount_aggressively(frm, newRow, amount, () => {
//                                 // 🔥 AUTOMATIC NEW ROW CALCULATION - PROPER SEQUENCE
//                                 console.log('🔄 Starting automatic new row calculations...');

//                                 // STEP 1: Refresh items first
//                                 frm.refresh_field('items');

//                                 // STEP 2: Update totals after items refresh
//                                 setTimeout(() => {
//                                     update_total_qty(frm);
//                                     calculate_document_totals(frm);

//                                     // STEP 3: Refresh UI fields
//                                     setTimeout(() => {
//                                         frm.refresh_field('total');
//                                         frm.refresh_field('net_total');

//                                         // STEP 4: Fix taxes with UOM logic
//                                         setTimeout(() => {
//                                             refresh_taxes_on_net_total_change(frm, frm.doc.net_total);

//                                             console.log('✅ New row calculations complete - Amount, Totals, and Taxes fixed');
...
//                                             frappe.show_alert({
//                                                 message: __(`Added ${uom} of ${itemName} (Amount: ${amount})`),
//                                                 indicator: 'green'
//                                             });
//                                         }, 300);
//                                     }, 200);
...
//                     // ✅ Get rate from Sales Order or fallback
//                     if (salesOrderRef) {
//                         frappe.db.get_doc('Sales Order', salesOrderRef).then(soDoc => {
//                             let soItem = null;

//                             if (so_detail_id) {
//                                 soItem = soDoc.items.find(i => i.name === so_detail_id);
//                             }

//                             if (!soItem) {
//                                 soItem = soDoc.items.find(i => i.item_code === itemCode);
//                             }

//                             if (soItem) {
//                                 const rateToUse = soItem.rate || 0;
//                                 // ✅ FIXED: Check both field names for free item status
...
//                                     },
//                                     callback: function (r) {
//                                         const fallbackRate = r.message ? (r.message.rate || 0) : 0;
//                                         handleRateAndUpdate(fallbackRate, false);
//                                     }
//                                 });
//                             }
//                         }).catch(err => {
...
//                 // ✅ Check if this item is marked as free in Sales Order
...
//                     // ✅ QUANTITY VALIDATION for free items
...
//                         // Check for existing free item row
//                         const existingRow = frm.doc.items.find(item =>
//                             item.item_code === itemCode &&
//                             item.batch_no === batchNo &&
//                             item.is_free_item === 1
...
//                                 newStockQty = newQty;
//                             } else {
//                                 newStockQty = newQty * conversionRate;
//                             }

//                             // 🔥 STORE CUSTOM AMOUNT for free items (always 0)
//                             if (!frm._custom_amounts) frm._custom_amounts = {};
...
//                             ]).then(() => {
//                                 // 🔥 AUTOMATIC FREE ITEM CALCULATION - PROPER SEQUENCE
//                                 setTimeout(() => {
//                                     update_total_qty(frm);
//                                     calculate_document_totals(frm);
...
// // 🔥 =============== NEW AGGRESSIVE AMOUNT PROTECTION FUNCTIONS ===============

// // 🔥 AMOUNT PROTECTION FUNCTION (Less aggressive to prevent loops)
...
// // 🔥 FUNCTION to restore custom values (removed - no longer needed)  
// // The restoration is now handled directly in calculate_taxes_and_totals override

// // 🔥 FUNCTION to fix all amounts manually
