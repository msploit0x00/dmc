...
#         )
# ========================================
# الملف: your_app_name/overrides/loan_repayment.py
# ========================================
# انسخ هذا الكود بالكامل في الملف
...
class CustomLoanRepayment(LoanRepayment):
    ...
    Loan Repayment مخصص
    الهدف: 
    - من Salary Slip: إنشاء GL Entry تلقائياً
    - يدوي (بعد الاستقالة): بدون GL Entry (Payment Entry يتولى الأمر)
    """

    def on_submit(self):
        """
        ✅ إنشاء GL Entry فقط إذا كان من Salary Slip
        ✅ تجاهل GL Entry إذا كان دفع يدوي (بعد الاستقالة)
        """
        if self.check_is_manual_payment():
            # دفع يدوي بعد الاستقالة - بدون GL Entry
            self.update_paid_amount_in_loan()
            self.set_status_in_loan()

            frappe.msgprint(
                _("تم حفظ سند السداد بدون قيد محاسبي. أنشئ Payment Entry لإكمال المعاملة."),
                alert=True,
                indicator="blue"
            )
        else:
            # من Salary Slip - إنشاء GL Entry (السلوك الافتراضي)
            super(CustomLoanRepayment, self).on_submit()
            frappe.msgprint(
                _("تم حفظ سند السداد. تم إنشاء القيد المحاسبي من Salary Slip."),
                alert=True,
                indicator="green"
            )
    ...
    def on_cancel(self):
        """
        ✅ إلغاء GL Entry فقط إذا كان موجوداً (من Salary Slip)
        """
        has_gl_entry = frappe.db.exists("GL Entry", {
            "voucher_type": "Loan Repayment",
            "voucher_no": self.name,
        ...
        if has_gl_entry:
            # من Salary Slip - إلغاء GL Entry
            super(CustomLoanRepayment, self).on_cancel()
        else:
            # يدوي - تحديث حالة القرض فقط
            self.update_paid_amount_in_loan()
            self.set_status_in_loan()
            self.update_npa_status_on_cancel()

        # إلغاء ربط Payment Entry إذا كان موجوداً
        if self.payment_entry:
            frappe.db.set_value("Loan Repayment", self.name, "payment_entry", None)

    def update_npa_status_on_cancel(self):
        """تحديث حالة NPA عند إلغاء سندات السداد اليدوية"""
        try:
            from lending.loan_management.doctype.loan_repayment.loan_repayment import (
                update_all_linked_loan_customer_npa_status
            )
            ...
            frappe.log_error(
                title=f"فشل تحديث NPA لـ {self.name}",
                message=str(e)
            )

    def make_gl_entries(self, cancel=0, adv_adj=0):
        """
        ✅ حرج: تجاوز لمنع GL Entry للدفعات اليدوية
        """
        if self.check_is_manual_payment():
            frappe.logger().info(
                f"تجاهل GL Entry لسند السداد اليدوي {self.name}. "
                f"سيتم إنشاء GL Entry عبر Payment Entry."
            )
            return None

        # من Salary Slip - إنشاء GL Entry
        return super(CustomLoanRepayment, self).make_gl_entries(cancel=cancel, adv_adj=adv_adj)
    ...
    def validate(self):
        ...
        if self.payment_entry and self.docstatus == 0:
            ...
            if pe_status == 1:
                frappe.msgprint(
                    _("سند السداد هذا مرتبط بـ Payment Entry محفوظ {0}").format(
                        frappe.bold(self.payment_entry)
                    ),
                    alert=True,
                    indicator="orange"
    ...
    def check_is_manual_payment(self):
        """
        ✅ التحقق إذا كان هذا دفع يدوي (بعد الاستقالة)
        
        Returns: True إذا كان دفع يدوي، False إذا كان من Salary Slip
        """
        # الطريقة 1: فحص الحقل المخصص
        manual_payment_flag = self.get('is_manual_payment')
        if manual_payment_flag and int(manual_payment_flag) == 1:
            return True

        # الطريقة 2: إذا كان payroll_payable_account موجود = من Salary Slip
        if self.payroll_payable_account:
            return False

        # الطريقة 3: فحص إذا كان مرتبطاً بـ Salary Slip
        salary_slip_loan = frappe.db.exists("Salary Slip Loan", {
            "loan_repayment_entry": self.name
        })
        if salary_slip_loan:
        ...
        # افتراضي: إذا لم يكن من salary slip = افترض يدوي
        return True

    def update_paid_amount_in_loan(self):
        """تحديث المبلغ المدفوع في مستند القرض"""
        if self.against_loan:
            total_paid = frappe.db.sql("""
                SELECT IFNULL(SUM(amount_paid), 0)
                FROM `tabLoan Repayment`
            ...
            frappe.db.set_value("Loan", self.against_loan, "total_amount_paid", total_paid)

    def set_status_in_loan(self):
        """تحديث حالة القرض بناءً على الدفع"""
        if self.against_loan:
            from lending.loan_management.doctype.loan.loan import Loan
            loan = frappe.get_doc("Loan", self.against_loan)
            loan.set_status()
...
def make_payment_entry(source_name, target_doc=None):
    ...
    ✅ هذا ينشئ GL Entry للدفعات اليدوية
    """
    def set_missing_values(source, target):
        loan_repayment = frappe.get_doc("Loan Repayment", source_name)
        ...
        # التحقق من الشروط
        if loan_repayment.docstatus != 1:
            frappe.throw(_("يجب حفظ Loan Repayment قبل إنشاء Payment Entry"))

        if loan_repayment.payment_entry:
            frappe.throw(
                _("Payment Entry {0} موجود بالفعل لهذا Loan Repayment").format(
                    frappe.bold(loan_repayment.payment_entry)
                )
            )
        ...
        if loan_repayment.payroll_payable_account and not is_manual:
            frappe.throw(
                _("لا يمكن إنشاء Payment Entry لسداد من Salary Slip. "
                  "تم إنشاء GL Entry بالفعل عبر Salary Slip.")
            )

        loan = frappe.get_doc("Loan", loan_repayment.against_loan)
        company = frappe.get_doc("Company", loan_repayment.company)

        # إعدادات Payment Entry
        target.payment_type = "Receive"
        target.party_type = "Employee"
        target.party = loan.applicant
        target.paid_amount = loan_repayment.amount_paid
        ...
        if not target.paid_to:
            frappe.throw(_("يرجى تعيين حساب النقدية الافتراضي في الشركة {0}").format(company.name))
        ...
        target.remarks = (
            f"دفع يدوي لسند السداد {loan_repayment.name} "
            f"مقابل القرض {loan.name} للموظف {loan.applicant} (بعد الاستقالة)"
        )
