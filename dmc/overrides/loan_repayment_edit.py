...
# ========================================
# ✅ CRITICAL FIX: Override Salary Slip Loan Repayment Entry Creation
# ========================================

def custom_make_loan_repayment_entry(doc):
    """
    🔥 ULTIMATE FIX: Block Loan Repayment Entry if loan paid manually

    This function is called by Salary Slip AFTER submit.
    We MUST check one final time if loans are paid via Payment Entry.
    """

    frappe.logger().info(
        f"🔍 FINAL CHECK: Salary Slip {doc.name} - Making Loan Repayment Entry")

    # ✅ CHECK 1: Skip flag in DB (persisted)
    skip_flag = frappe.db.get_value(
        "Salary Slip", doc.name, "custom_skip_loan_repayment_creation")
    if flt(skip_flag) == 1:
        frappe.logger().info(f"🚫 BLOCKED: {doc.name} - skip flag set in DB")
        return None

    # ✅ CHECK 2: Empty loans table
    if not doc.get("loans") or len(doc.loans) == 0:
        frappe.logger().info(f"🚫 BLOCKED: {doc.name} - loans table empty")
        return None

    # ✅ CHECK 3: Re-verify each loan is NOT paid manually
    valid_loans = []
    blocked_loans = []
    ...
    for loan_row in doc.loans:
        ...
        """, (loan_row.loan, doc.posting_date))[0][0] or 0

        if has_manual_payment > 0:
            blocked_loans.append(loan_row.loan)
            frappe.logger().warning(
                f"⚠️ BLOCKED: Loan {loan_row.loan} was paid MANUALLY via Payment Entry"
            )
            continue

        # Check Active schedule - no unpaid installments in this period
        active_schedule = frappe.db.sql("""
            SELECT name
            FROM `tabLoan Repayment Schedule`
            WHERE loan = %s
        ...
        if unpaid_count == 0:
            blocked_loans.append(loan_row.loan)
            frappe.logger().warning(
                f"⚠️ BLOCKED: Loan {loan_row.loan} - NO unpaid installments in period"
            )
            continue

        valid_loans.append(loan_row)

    # ✅ CHECK 4: After filtering, are there any valid loans left?
    if not valid_loans:
        frappe.logger().warning(
            f"🚫 BLOCKED: All loans in {doc.name} are either paid manually or fully paid"
        )
            ...
            frappe.msgprint(
                _("⚠️ Loan Repayment Entry NOT created.<br><br>"
                  "The following loans were paid manually via Payment Entry:<br>{0}").format(
                    "<br>".join(
                        [f"- {frappe.bold(loan)}" for loan in blocked_loans])
                ),
                alert=True,
                indicator="orange",
    ...
    # ✅ All checks passed - create repayment ONLY for valid loans
    frappe.logger().info(
        f"✅ Creating Loan Repayment Entry for {doc.name} with {len(valid_loans)} valid loan(s)"
    )
...
def prevent_duplicate_loan_deduction(doc, method):
    ...
    loans_with_manual_payment = frappe.db.sql("""
        ...
        AND pe.custom_is_manual_loan_payment = 1
        AND lr.posting_date <= %s
    """, (doc.employee, doc.company, doc.posting_date), as_dict=1)

    manual_loan_set = {loan.loan for loan in loans_with_manual_payment}

    # ✅ STEP 2: Get loans with unpaid installments in ACTIVE schedule
    loans_with_unpaid = frappe.db.sql("""
        SELECT DISTINCT 
            lrs.loan,
            lrs.name as schedule_name,
    ...
    # ✅ STEP 3: Filter out manually paid loans
    loans_with_unpaid = [
        loan for loan in loans_with_unpaid
        if loan.loan not in manual_loan_set
    ]
    ...
    if not loans_with_unpaid:
        ...
        if manual_loan_set:
            frappe.msgprint(
                _("⚠️ القروض التالية تم دفعها يدوياً (عن طريق Payment Entry) ولن يتم خصمها:<br>{0}").format(
                    "<br>".join(
                        [f"- {frappe.bold(loan)}" for loan in manual_loan_set])
                ),
                alert=True,
                indicator="blue",
    ...
    if not valid_rows:
        ...
        if manually_paid_loans:
            frappe.msgprint(
                _("⚠️ القروض المدفوعة يدوياً (تم تجاهلها):<br>{0}").format(
                    "<br>".join(
                        [f"- {frappe.bold(loan)}" for loan in manually_paid_loans])
                ),
                alert=True,
...
def get_monthly_repayment_amount(loan_id):
    ...
    active_schedule = frappe.get_all(
        "Loan Repayment Schedule",
        filters={"loan": loan_id, "status": "Active", "docstatus": 1},
        fields=["name"],
        order_by="posting_date desc",
        limit=1
    )
