...
@frappe.whitelist()
def get_monthly_repayment_amount(loan_id):
    """Get monthly repayment amount from Loan Repayment Schedule"""
    if not loan_id:
        return 0
...
# ========================================
# 🔥 Salary Slip Functions (ULTIMATE FIX)
# ========================================

def prevent_duplicate_loan_deduction(doc, method):
    """
    ✅ ULTIMATE FIX: Prevent duplicate loan deductions
    """
    if not doc.employee:
        return

    frappe.logger().info(
        f"🔍 Checking loans for {doc.name} (Period: {doc.start_date} to {doc.end_date})"
    )

    # ✅ STEP 1: Get loans with unpaid installments from ACTIVE schedules ONLY
    loans_with_unpaid = frappe.db.sql("""
        SELECT DISTINCT
            lrs.loan,
            lrs.name as schedule_name,
    ...
    # ✅ STEP 2: If NO unpaid installments found
    if not loans_with_unpaid:
        frappe.logger().info(
            f"🚫 {doc.name}: No unpaid installments in Active schedules"
        )
    ...
    # ✅ STEP 3: Create dict of unpaid loans
    loans_unpaid_dict = {loan.loan: loan for loan in loans_with_unpaid}
    ...
    if not valid_rows:
        ...
        frappe.logger().info(
            f"🚫 {doc.name}: All loans removed - forced ZERO amounts"
        )
    else:
        # Clear skip flag
        doc.custom_skip_loan_repayment_creation = 0
        ...
        frappe.logger().info(
            f"✅ {doc.name}: Keeping {len(valid_rows)} loan(s), "
            f"Total Loan Repayment: {doc.total_loan
