...
def execute(filters=None):
    ...
    # If shipment filter is applied, aggregate by item_code (sum expenses from multiple vouchers)
    if filters.get("shipment_name"):
        items_data = aggregate_items_by_item_code(items_data, expense_accounts)
...
def aggregate_items_by_item_code(items_data, expense_accounts):
    ...
    for item_data in items_data:
        ...
        if item_code not in aggregated:
            ...
            aggregated[item_code]['voucher_names'] = [
                item_data['landed_cost_voucher']]
        else:
            # Subsequent occurrences - only sum the expense_allocations
            for account_code in expense_accounts.keys():
                aggregated[item_code]['expense_allocations'][account_code] += item_data['expense_allocations'].get(
            ...
            # Add voucher name to the list
            if item_data['landed_cost_voucher'] not in aggregated[item_code]['voucher_names']:
                aggregated[item_code]['voucher_names'].append(
                    item_data['landed_cost_voucher'])

    # Convert voucher_names list to comma-separated string
    for item_code in aggregated:
        aggregated[item_code]['landed_cost_voucher'] = ', '.join(
            aggregated[item_code]['voucher_names'])
...
def get_items_with_raw_calculations(filters, expense_accounts):
    ...
    # Add shipment filter at SQL level
    shipment_filter_sql = ""
    if filters.get("shipment_name"):
        # Get available shipment fields dynamically
        available_fields = get_available_shipment_fields()
        if available_fields:
            # Build OR condition for all available shipment fields
            shipment_conditions = " OR ".join(
                [f"lcv.{field} = %(shipment_name)s" for field in available_fields])
            shipment_filter_sql = f"AND ({shipment_conditions})"

    # Get vouchers that match the filters
    try:
        vouchers_sql = f"""
            SELECT 
                lcv.name,
...
def convert_to_horizontal_display_raw_values(items_data, expense_accounts):
    ...
    # Process each item directly - no grouping or aggregation
    for item_data in items_data:
        row = {
            'landed_cost_voucher': item_data['landed_cost_voucher'],
            'purchase_receipt': item_data['purchase_receipt'],
