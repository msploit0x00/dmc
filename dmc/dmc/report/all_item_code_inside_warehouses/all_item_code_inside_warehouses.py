...
#     if warehouse_filter:
#         # Show only the selected warehouse column
#         columns.append({
#             "fieldname": warehouse_filter,
#             "label": _(warehouse_filter),
#             "fieldtype": "Float",
...
#         })
#     else:
#         # Show all warehouses if no filter is applied
#         warehouses = frappe.get_all("Warehouse", fields=["name"])
#         for warehouse in warehouses:
#             columns.append({
#                 "fieldname": warehouse.name,
...
#         row = {"item_group": item_group.name}
#         for warehouse in warehouses:
#             actual_qty_sum = frappe.db.sql("""
#                 SELECT SUM(sle.actual_qty) as total_qty
#                 FROM `tabStock Ledger Entry` sle
#                 JOIN `tabItem` item ON sle.item_code = item.name
#                 WHERE item.item_group = %s AND sle.warehouse = %s
#             """, (item_group.name, warehouse.name), as_dict=True)[0].total_qty or 0  # Ensure sum is never None
