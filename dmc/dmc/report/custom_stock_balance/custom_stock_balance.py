...
class StockBalanceReport:
    ...
    def prepare_opening_data_from_closing_balance(self) -> None:
        ...
        res = frappe.get_doc("Closing Stock Balance",
                             closing_balance[0].name).get_prepared_data()
    ...
    def prepare_new_data(self):
        ...
        if self.filters.get("show_stock_ageing_data"):
            ...
            item_wise_fifo_queue = FIFOSlots(
                self.filters, self.sle_entries).generate()
        ...
        for _key, report_data in self.item_warehouse_map.items():
            ...
            if self.filters.get("show_stock_ageing_data"):
                opening_fifo_queue = self.get_opening_fifo_queue(
                    report_data) or []
                ...
                stock_ageing_data = {"average_age": 0,
                                     "earliest_age": 0, "latest_age": 0}
                if opening_fifo_queue:
                    fifo_queue = sorted(
                        filter(_func, opening_fifo_queue), key=_func)
                    if not fifo_queue:
                        continue
    ...
    def get_item_warehouse_map(self):
        ...
        with frappe.db.unbuffered_cursor():
            if not self.filters.get("show_stock_ageing_data"):
                self.sle_entries = self.sle_query.run(
                    as_dict=True, as_iterator=True)

            for entry in self.sle_entries:
                group_by_key = self.get_group_by_key(entry)
                if group_by_key not in item_warehouse_map:
                    self.initialize_data(
                        item_warehouse_map, group_by_key, entry)

                self.prepare_item_warehouse_map(
                    item_warehouse_map, entry, group_by_key)
    ...
    def get_closing_balance(self) -> list[dict[str, Any]]:
        ...
        for fieldname in ["warehouse", "item_code", "item_group", "warehouse_type"]:
            if self.filters.get(fieldname):
                query = query.where(
                    table[fieldname] == self.filters.get(fieldname))
    ...
    def apply_inventory_dimensions_filters(self, query, sle) -> str:
        ...
        if inventory_dimension_fields:
            for fieldname in inventory_dimension_fields:
                ...
                if self.filters.get(fieldname):
                    query = query.where(sle[fieldname].isin(
                        self.filters.get(fieldname)))
    ...
    def apply_items_filters(self, query, item_table) -> str:
        if item_group := self.filters.get("item_group"):
            ...
            query = query.where(
                item_table.item_group.isin([*children, item_group]))
        ...
        for field in ["item_code", "brand"]:
            if not self.filters.get(field):
                ...
                query = query.where(
                    item_table[field] == self.filters.get(field))
    ...
    def get_columns(self):
        ...
        columns.extend(
            [
                ...
                {"label": _("In Value"), "fieldname": "in_val",
                ...
                {"label": _("Out Value"), "fieldname": "out_val",
                 "fieldtype": "Float", "width": 80},
                {
        ...
        if self.filters.get("show_stock_ageing_data"):
            columns += [
                {"label": _("Average Age"),
                 "fieldname": "average_age", "width": 100},
                {"label": _("Earliest Age"),
                 "fieldname": "earliest_age", "width": 100},
                {"label": _("Latest Age"),
                 "fieldname": "latest_age", "width": 100},
            ]
