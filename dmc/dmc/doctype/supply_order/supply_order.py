...
class Supplyorder(SellingController):
    ...
    if TYPE_CHECKING:
        ...
        source: DF.Link | None
        status: DF.Literal["Draft", "Open", "Replied",
    ...
    def validate_valid_till(self):
        if self.valid_till and getdate(self.valid_till) < getdate(self.transaction_date):
            frappe.throw(
                _("Valid till date cannot be before transaction date"))
    ...
    def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
        if not (self.is_fully_ordered() or self.is_partially_ordered()):
            get_lost_reasons = frappe.get_list(
                "Quotation Lost Reason", fields=["name"])
            lost_reasons_lst = [reason.get("name")
                                for reason in get_lost_reasons]
    ...
    def on_submit(self):
        ...
        self.update_opportunity("Supplyorder")
        self.update_lead()

    def on_cancel(self):
        if self.lost_reasons:
            self.lost_reasons = []
        super(Supplyorder, self).on_cancel()
        ...
        if not frappe.db.exists("Customer", quotation.get("party_name")):
            ...
            if not customer_name:
                ...
                try:
                    ...
                    mandatory_fields = [customer.meta.get_label(
                    ...
                    message += "<br><ul><li>" + \
                        "</li><li>".join(mandatory_fields) + "</li></ul>"
